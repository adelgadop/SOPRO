...
mod1 = pkl.load(open('02_data/processed/res02_b_chem_d02_2017_'+month+'.pkl', 'rb')) # 3 km WRF-Chem v3.9.1
mod2 = pkl.load(open('02_data/processed/lisboa_amanan_d02_2017_'+month+'.pkl', 'rb')) # 5 km WRF-Chem v4.2.1

stations = pd.read_csv('02_data/raw/air_quality/stations.csv')
codes = list(mod2.keys())
...
#%% Make Plots
def plots_by_station(nombre, all_df1, all_df2):
    fig, ax = plt.subplots(3,2, figsize=[12,8], sharex=True, gridspec_kw={'hspace':0.1})
    name=nombre
    plt.suptitle("Estação "+name)

    # PM10
    all_df1.loc[all_df1.name == name, :].plot(x='local_date', y=['pm10_obs'], style='-ok', lw=1, ax=ax[0,0])
    all_df1.loc[all_df1.name == name, :].plot(x='local_date', y=['pm10_mod1'], style=['-g'], lw=4, alpha=.7, ax=ax[0,0])
    all_df2.loc[all_df2.name == name, :].plot(x='local_date', y=['pm10_mod2'], style=['-b'], lw=4, alpha=.7, ax=ax[0,0])
    ax[0,0].legend(['Obs', '3 km (WRF-Chem v3.9.1)', '5 km (WRF-Chem v4.2.1)'], fontsize=14, bbox_to_anchor=(2.1, -0.35))
    ...
    all_df1.loc[all_df1.name == name, :].plot(x='local_date', y=['pm2.5_mod1'], style=['-g'], lw=4, alpha=.7, ax=ax[0,1])
    all_df2.loc[all_df2.name == name, :].plot(x='local_date', y=['pm2.5_mod2'], style=['-b'], lw=4, alpha=.7, ax=ax[0,1])
    ...
    all_df1.loc[all_df1.name == name, :].plot(x='local_date', y=['co_mod1'], style=['-g'], lw=4, alpha=.7, ax=ax[1,0])
    all_df2.loc[all_df2.name == name, :].plot(x='local_date', y=['co_mod2'], style=['-b'], lw=4, alpha=.7, ax=ax[1,0])
    ...
    all_df1.loc[all_df1.name == name, :].plot(x='local_date', y=['o3_mod1'], style=['-g'], lw=4, alpha=.7, ax=ax[2,1])
    all_df2.loc[all_df2.name == name, :].plot(x='local_date', y=['o3_mod2'], style=['-b'], lw=4, alpha=.7, ax=ax[2,1])
    ...
    all_df1.loc[all_df1.name == name, :].plot(x='local_date', y=['no2_mod1'], style=['-g'], lw=4, alpha=.7, ax=ax[2,0])
    all_df2.loc[all_df2.name == name, :].plot(x='local_date', y=['no2_mod2'], style=['-b'], lw=4, alpha=.7, ax=ax[2,0])
...
for n in stations.name:
    plots_by_station(n, all_df1, all_df2)
